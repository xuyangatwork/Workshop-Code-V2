...
def call_cohere_api(full_prompt):
	...
	with st.status("Calling the Cohere API..."):
		# Call the Cohere API
		response = co.generate(prompt=full_prompt, max_tokens=1000, temperature=st.session_state.temp, presence_penalty=st.session_state.presence_penalty, frequency_penalty=st.session_state.frequency_penalty)
