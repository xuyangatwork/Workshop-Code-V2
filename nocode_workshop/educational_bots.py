...
# 	relations = []
# 	for ent in doc.ents:
# 		# Assuming the subject is the named entity and the object can be found as the root of the sentence
# 		subject = ent.text
# 		for token in ent.root.head.children:
# 			if token.dep_ in ('dobj', 'pobj', 'attr') and token.pos_ in ('NOUN', 'PROPN'):
# 				relations.append((subject, ent.root.head, token))
...
# 		G.add_edge(rel[0], rel[2].text, relationship=rel[1].text)
# 	return G

# def plot_knowledge_graph(G):
#     pos = nx.spring_layout(G)
#     plt.figure(figsize=(12, 8))
#     nx.draw(G, pos, with_labels=True, node_color='skyblue', node_size=2000, edge_color='k', linewidths=1, font_size=10, arrows=True, arrowstyle='->', arrowsize=10)
#     nx.draw_networkx_edge_labels(G, pos, edge_labels=nx.get_edge_attributes(G,'relationship'))
    
#     st.pyplot(plt)

# def show_knowledge_graph(text):
